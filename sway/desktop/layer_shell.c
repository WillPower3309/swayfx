...
static void arrange_surface(struct sway_output *output, const struct wlr_box *full_area,
	...
	wl_list_for_each(node, &tree->children, link) {
		...
		const int surface_width = surface->layer_surface->surface->current.width;
		const int surface_height = surface->layer_surface->surface->current.height;

		wlr_scene_node_set_enabled(&surface->blur_node->node, surface->blur_enabled);
		wlr_scene_blur_set_size(surface->blur_node, surface_width, surface_height);

		wlr_scene_node_set_enabled(&surface->box_shadow_node->node,
				surface->shadow_enabled && !surface->use_drop_shadow);
		wlr_scene_node_set_enabled(&surface->drop_shadow_node->node,
				surface->shadow_enabled && surface->use_drop_shadow);

		if (surface->shadow_enabled) {
			if (surface->use_drop_shadow) {
				// Adjust the size and position of the shadow node
				const int blur_offset = wlr_scene_drop_shadow_get_offset(surface->drop_shadow_node);
				wlr_scene_drop_shadow_set_size(surface->drop_shadow_node,
						surface_width + blur_offset * 2,
						surface_height + blur_offset * 2);
				...
				wlr_scene_shadow_set_size(surface->box_shadow_node,
						surface_width + config->shadow_blur_sigma * 2,
						surface_height + config->shadow_blur_sigma * 2);
				const int x = config->shadow_offset_x - config->shadow_blur_sigma;
				const int y = config->shadow_offset_y - config->shadow_blur_sigma;
				wlr_scene_node_set_position(&surface->box_shadow_node->node, x, y);

				wlr_scene_shadow_set_clipped_region(surface->box_shadow_node, (struct clipped_region) {
...
static struct sway_layer_surface *sway_layer_surface_create(
	...
	surface->box_shadow_node = alloc_scene_shadow(surface->tree, 0, 0,
			0, config->shadow_blur_sigma, config->shadow_color, &failed);
	surface->drop_shadow_node = alloc_scene_drop_shadow(surface->tree, 0, 0,
...
static void handle_map(struct wl_listener *listener, void *data) {
	...
	wlr_scene_node_lower_to_bottom(&surface->box_shadow_node->node);
	wlr_scene_node_lower_to_bottom(&surface->drop_shadow_node->node);
